...
import {
  Clock, Trophy, Users, FileText, PenLine, Lock,
} from 'lucide-react';
...
export default async function PromptDetailPage({ params }: Props) {
  ...
  const isEnded    = prompt.status === 'ENDED' || new Date(prompt.endDate) < new Date();
  ...
  return (
    <div className="px-4 py-6 md:px-8 max-w-4xl mx-auto">
      ...
      <div className="mt-6 rounded-2xl bg-[#252525] border border-[#2a2a2a] shadow-xl overflow-hidden">
        <div className={`h-1.5 w-full ${isEnded ? 'bg-white/10' : 'bg-[#FFC300]'}`} />
        <div className="p-6 md:p-8">
            ...
            {isCreator && (
              <div className="flex items-center gap-2">
                <Link
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium bg-white/5 text-white/70 border border-[#333] hover:text-white hover:border-[#FFC300]/40 transition-all"
                >
          ...
          {/* Meta row */}
          <div className="flex flex-wrap items-center gap-5 text-sm text-white/70 border-t border-[#2a2a2a] pt-5">
            <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center gap-1.5 ${isEnded ? '' : 'text-[#FFC300]/80'}`}>
              {isEnded ? <Trophy className="w-4 h-4" /> : <Clock className="w-4 h-4" />}
              {isEnded ? `Ended ${formatDate(prompt.endDate)}` : formatCountdown(prompt.endDate)}
            </div>
          </div>
        </div>
      </div>
      ...
      {(acceptedParticipants.length > 0 || isCreator) && (
        <div className="mt-6 p-5 rounded-2xl bg-[#1e1e1e] border border-[#2a2a2a]">
          <h2 className="text-xs font-semibold text-white/70 uppercase tracking-wider mb-3 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-white/70">{creatorName}</p>
                <p className="text-[10px] text-[#FFC300]/60">Creator</p>
              </div>
            </div>
            {acceptedParticipants.map((inv) => {
              const name = [inv.user.firstName, inv.user.lastName].filter(Boolean).join(' ') || inv.user.username || 'Unknown';
              return (
      ...
      {myInvite === 'PENDING' && !isCreator && (
        <div className="mt-6 p-4 rounded-2xl bg-[#FFC300]/5 border border-[#FFC300]/20 flex items-center justify-between gap-4">
          <p className="text-sm text-[#FFC300]">You have been invited to this challenge!</p>
      ...
      {canSubmit && (
        <div className="mt-6 p-5 rounded-2xl bg-[#1e1e1e] border border-[#FFC300]/20 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-white">Ready to participate?</p>
            <p className="text-xs text-white/70 mt-0.5">Write and submit your entry before the deadline.</p>
          </div>
          <Link
      ...
      {/* Entries section (only visible when ended) */}
      {isEnded && (
        <div className="mt-8">
          <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
            <Trophy className="w-5 h-5 text-[#FFC300]" />
            Entries ({entries.length})
          </h2>
          <EntryList entries={entries} promptId={promptId} currentUserId={userId ?? null} />
        </div>
      )}

      {/* Active — show entry count but not entries */}
      {!isEnded && (
        <div className="mt-8 rounded-xl border border-dashed border-[#2a2a2a] py-8 text-center">
          <Clock className="w-6 h-6 text-[#FFC300]/40 mx-auto mb-2" />
          <p className="text-sm text-white/70">
            {prompt.entryCount} entr{prompt.entryCount !== 1 ? 'ies' : 'y'} submitted so far.
          </p>
