...
export function PromptGrid({ prompts }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'active') list = list.filter((p) => p.status === 'ACTIVE' && p.endDate > new Date());
    if (filter === 'ended')  list = list.filter((p) => p.status === 'ENDED' || p.endDate <= new Date());

    if (sort === 'newest')       list.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    if (sort === 'ending-soon')  list.sort((a, b) => a.endDate.getTime() - b.endDate.getTime());
    if (sort === 'most-entries') list.sort((a, b) => b.entryCount - a.entryCount);
  ...
  if (prompts.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-[#2a2a2a] bg-[#1a1a1a]/40 py-20 text-center">
        ...
        <p className="text-sm text-white/70 mb-6">Create your first writing challenge and invite friends!</p>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-1 p-1 rounded-xl bg-[#1e1e1e] border border-[#2a2a2a]">
          {(['all', 'active', 'ended'] as FilterOption[]).map((f) => (
            ...
            >
              {f === 'all' ? `All (${prompts.length})` : f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>
        ...
        <select
          ...
          className="ml-auto px-3 py-1.5 rounded-xl bg-[#1e1e1e] border border-[#2a2a2a] text-sm text-white/70 focus:outline-none focus:border-[#FFC300]/40 transition-all"
        >
