...
const PREMIUM_FEATURES = [
  { label: 'Unlimited books & chapters', included: true },
  { label: 'Unlimited book clubs', included: true },
  { label: 'Create & join unlimited hives', included: true },
  { label: 'Participate in writing prompts', included: true },
  { label: 'Unlimited reading lists', included: true },
...
const HIGHLIGHTS = [
  {
    ...
    description: 'No caps on books, chapters, or words. Write as much as you want.',
  },
  {
    icon: Users2,
    color: 'text-blue-400',
    ...
    description: 'Join and create as many book clubs as you like.',
  },
  {
    icon: Hexagon,
    color: 'text-teal-400',
    bg: 'bg-teal-400/10',
    title: 'Full Hive Access',
    description: 'Collaborate on unlimited hives with writers across the community.',
  },
  {
    icon: PenLine,
    ...
    description: 'Participate in all prompts and share your entries with everyone.',
  },
  {
    icon: Star,
    ...
    bg: 'bg-purple-400/10',
    title: 'Premium Badge',
    description: 'Stand out on your profile with an exclusive premium badge.',
  },
  {
    icon: Zap,
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 md:px-8">
      ...
      <div className="relative rounded-2xl border border-[#FFC300]/30 bg-[#FFC300]/5 p-8 mb-10 text-center overflow-hidden">
        ...
        <div className="relative">
          ...
          <button
            className="w-full max-w-xs mx-auto flex items-center justify-center gap-2 px-8 py-3.5 rounded-full bg-[#FFC300] text-black font-bold text-base hover:bg-[#FFD54F] transition-colors"
          >
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {HIGHLIGHTS.map(({ icon: Icon, color, bg, title, description }) => (
            <div
              key={title}
              className="flex items-start gap-4 p-4 rounded-2xl bg-[#252525] border border-[#2a2a2a]"
            >
      ...
      <div>
        ...
        <div className="rounded-2xl bg-[#252525] border border-[#2a2a2a] overflow-hidden">
          ...
          {FREE_FEATURES.map(({ label, included }, i) => (
            <div
              key={label}
              className={`grid grid-cols-3 ${i < FREE_FEATURES.length - 1 ? 'border-b border-[#2a2a2a]' : ''}`}
            >
          ...
          {/* Premium-only rows */}
          {PREMIUM_FEATURES.filter(f => !FREE_FEATURES.find(ff => ff.label === f.label)).map(({ label }, i) => (
            <div
              key={label}
              className={`grid grid-cols-3 border-t border-[#2a2a2a]`}
            >
              <div className="px-5 py-3 col-span-1">
                <span className="text-sm text-white">{label}</span>
              </div>
      ...
      <div className="mt-10 rounded-2xl border border-[#2a2a2a] bg-[#252525] p-7 text-center">
        ...
        <p className="text-sm text-white/80 mb-5">
          Join thousands of writers on Beehive Premium.
        </p>
        <button className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-[#FFC300] text-black font-bold text-sm hover:bg-[#FFD54F] transition-colors">
          <Sparkles className="w-4 h-4" />
          Get Premium — $5/mo
        </button>
      </div>
