...
function Avatar({ user, size = 28 }: { user: ChatMessageWithAuthor['author']; size?: number }) {
...
export default function HiveChat({
  ...
  const canDelete = (msg: ChatMessageWithAuthor) =>
    msg.authorId === currentUserId || myRole === 'OWNER' || myRole === 'MODERATOR';

  return (
    <div className="flex flex-col rounded-2xl border border-[#2a2a2a] overflow-hidden" style={{ height: 'calc(100vh - 220px)', minHeight: 480 }}>
      {/* Message list */}
              ...
              >
                ...
                <div className={`max-w-[72%] flex flex-col gap-0.5 ${isOwn ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className="flex items-center gap-1.5 group/bubble">
                    ...
                    <div
                      className={`px-3 py-2 rounded-2xl text-sm text-white leading-relaxed break-words whitespace-pre-wrap ${
                        isOwn
                  ...
                  {/* Timestamp — show on last in a run or if standalone */}
                  {(!messages[idx + 1] || messages[idx + 1]?.authorId !== msg.authorId) && (
                    <span className={`text-[10px] text-white/25 px-1 ${isOwn ? 'text-right' : ''}`}>
