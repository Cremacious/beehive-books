...
import type { ReadingListFormData, BookEntryData } from '@/lib/validations/reading-list.schema';
...
interface ReadingListStore {
  ...
  createList:          (data: ReadingListFormData, initialBooks: BookEntryData[], currentlyReadingIdx?: number | null) => Promise<ActionResult & { listId?: string }>;
  updateList:          (listId: string, data: ReadingListFormData) => Promise<ActionResult>;
  ...
  toggleReadStatus:    (listId: string, bookId: string, currentStatus: boolean) => Promise<void>;
  setCurrentlyReading: (listId: string, bookId: string | null, previousBookId: string | null) => Promise<void>;
}
...
export const useReadingListStore = create<ReadingListStore>((set) => ({
  ...
  updateList: (listId, data) =>
    updateReadingListAction(listId, data),

  deleteList: (listId) =>
    deleteReadingListAction(listId),

  addBook: (listId, bookData) =>
    addBookToListAction(listId, bookData),

  removeBook: (listId, bookId) =>
    removeBookFromListAction(listId, bookId),
  ...
  toggleReadStatus: async (listId, bookId, currentStatus) => {
    ...
    if (!result.success) {
      set((s) => ({
        optimisticReadStatus: { ...s.optimisticReadStatus, [bookId]: currentStatus },
      }));
    }
  },
  ...
  setCurrentlyReading: async (listId, bookId, previousBookId) => {
    set((s) => ({
      optimisticCurrentlyReading: { ...s.optimisticCurrentlyReading, [listId]: bookId },
    }));
    const result = await setCurrentlyReadingAction(listId, bookId);
    if (!result.success) {
      set((s) => ({
        optimisticCurrentlyReading: { ...s.optimisticCurrentlyReading, [listId]: previousBookId },
      }));
    }
  },
}));
