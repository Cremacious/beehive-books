...
function ChapterRow({
  ...
  const canManage = isMyClaim || myRole === 'OWNER' || myRole === 'MODERATOR';
      ...
      {claim ? (
        <div className="flex items-center gap-2 shrink-0">
          ...
          {canManage ? (
            ...
            <span
              className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${STATUS_CONFIG[claim.status].bg} ${STATUS_CONFIG[claim.status].color}`}
            >
      ...
      <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
        ...
        {myRole === 'OWNER' && (
          <Button size="sm" variant="outline" asChild>
            <a href={`/hive/${hiveId}/settings`}>Go to Settings</a>
          </Button>
        )}
      </div>
    );
